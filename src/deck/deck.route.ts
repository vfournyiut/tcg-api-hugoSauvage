...
decksRoute.post('/', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    if (!userId) {
      return res
        .status(400)
        .json({ error: "l'id de l'utilisateur est invaide" })
    }
    ...
    if (existingCards.length !== 10) {
      return res
        .status(400)
        .json({ error: 'Une ou plusieurs cartes sont invalides' })
    }
    ...
    const deck = await prisma.deck.create({
      data: {
        ...
        userId,
        cards: {
          create: existingCards.map((existingCards) => ({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (existingDeck.userId !== userId) {
        return res
          .status(403)
          .json({ error: 'Accès interdit : ce deck ne vous appartient pas' })
      }
      ...
      if (cards !== undefined) {
        ...
        if (!Array.isArray(cards)) {
          return res
            .status(400)
            .json({ error: 'La liste des cartes est invalide' })
        }
